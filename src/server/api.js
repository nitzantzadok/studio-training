/**
 * ה-API של הסטודיו — נתיבים בלבד, בלי שרת ובלי מערכת קבצים.
 *
 * זה הלב המשותף לשתי צורות ההרצה: שרת Node על מחשב או בענן, ופונקציה
 * בקצה (Cloudflare Workers) מול מסד מנוהל. שתיהן עוטפות את אותה טבלת
 * נתיבים, ולכן אין שתי מערכות שצריך לתחזק במקביל ואין התנהגות שקיימת
 * רק באחת מהן.
 *
 * מה שנשאר לעוטף: לקרוא את גוף הבקשה, להוציא את העוגייה, ולהחזיר תשובה.
 */
...
/*
 * המסד מוזרק על ידי העוטף. בשרת Node הוא נטען פעם אחת מקובץ; בקצה הוא
 * נטען מהמסד המנוהל בכל בקשה ונשמר בסופה.
 */
let db = null;
...
/** האם החיבור מוצפן — קובע אם עוגיית המושב מסומנת Secure. */
export const isSecureRequest = (proto) => String(proto || '').split(',')[0].trim() === 'https';

/** מזהה את החשבון מהעוגייה. מחזיר null כשאין מושב תקף — בלי לזרוק. */
export function resolveAccount(cookieHeader) {
...
const authRoutes = {
  ...
  'POST /api/auth/login': async (body, _url, ctx) => {
    ...
    if (!(await verifyPassword(String(body.password || ''), account))) {
  ...
  'POST /api/auth/logout': async (_body, _url, ctx) => {
    ...
    ctx.setCookie = clearCookie({ secure: ctx.secure });
    return { ok: true };
  },
  ...
  'POST /api/auth/password': async (body, _url, ctx) => {
    ...
    if (!(await verifyPassword(String(body.current || ''), ctx.account))) {
      return { ok: false, errors: ['הסיסמה הנוכחית שגויה.'] };
    }
    const p = validatePassword(body.next);
    if (!p.ok) return { ok: false, errors: p.errors };
    Object.assign(ctx.account, await hashPassword(body.next), { passwordChangedAt: new Date().toISOString() });
